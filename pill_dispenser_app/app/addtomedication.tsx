...
const MedicationDetailScreen = () => {
  ...
  // ── Pre-fill from prescription if available ─────────────────────────────
  const prefilledName = typeof params.prefilledName === 'string' ? params.prefilledName : ''
  const prefilledDays = params.prefilledDays
    ? JSON.parse(params.prefilledDays as string)
    : []

  const [medicineName, setMedicineName] = useState(prefilledName)
  const [selectedDays, setSelectedDays] = useState<string[]>(prefilledDays)
  ...
  const addDosageRow = () => {
    setDosages([...dosages, { id: Date.now(), amount: 1, time: "08:00" }])
  }
  ...
  const openTimePicker = (index) => {
    ...
    setPickerDate(parseTimeString(currentTime))
    setShowPicker(true)
  }
  ...
  const onTimeChange = (event, selectedDate) => {
    ...
    if (event.type === "set" && selectedDate) {
      ...
      if (editingDosageIndex >= 0) {
        ...
        newDosages[editingDosageIndex].time = newTime
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.sectionContainer}>
        ...
        <View style={styles.quickSelectRow}>
          ...
          <TouchableOpacity
            style={[styles.quickSelectButton, { backgroundColor: '#eee' }]}
            ...
            <View style={styles.dosageControl}>
              ...
              <View style={styles.dosageButtons}>
                <TouchableOpacity onPress={() => {
                  const newDosages = [...dosages]; newDosages[index].amount++; setDosages(newDosages)
                }}>
...
const styles = StyleSheet.create({
  ...
  sectionTitle: {
    ...
    marginBottom: 16, color: "#444",
  },
  quickSelectRow: {
    flexDirection: "row", justifyContent: "center", marginBottom: 12, gap: 8,
  },
  quickSelectButton: {
    backgroundColor: "#CC7755", paddingVertical: 6, paddingHorizontal: 14,
    borderRadius: 20,
  },
  ...
  scheduleHeader: {
    ...
    borderBottomWidth: 1, borderBottomColor: "#eee", paddingBottom: 5,
  },
  scheduleHeaderText: {
    fontSize: 14, fontWeight: "500", color: "#666", flex: 1, textAlign: "center",
  },
  dosageRow: {
    flexDirection: "row", justifyContent: "space-between",
  ...
  timeButton: {
    backgroundColor: "#CC7755", borderRadius: 8, paddingVertical: 12,
    paddingHorizontal: 10, flex: 2, flexDirection: "row",
